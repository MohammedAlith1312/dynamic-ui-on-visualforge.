...
interface PublicPageData {
    ...
    allPages: Page[];
    widgetData: Record<string, { rows: any[]; components: any[] }>;
}

interface PublicPageComponentProps {
    slug?: string;
    initialData?: PublicPageData;
}

const PublicPageComponent = ({ slug, initialData }: PublicPageComponentProps) => {
    const [page, setPage] = useState<Page | null>(initialData?.page || null);
    const [rows, setRows] = useState<PageRow[]>(initialData?.rows || []);
    const [components, setComponents] = useState<PageComponent[]>(initialData?.components || []);
    const [layout, setLayout] = useState<Layout | null>(initialData?.layout || null);
    const [layoutRows, setLayoutRows] = useState<LayoutRow[]>(initialData?.layoutRows || []);
    const [layoutComponents, setLayoutComponents] = useState<LayoutComponent[]>(initialData?.layoutComponents || []);
    const [allPages, setAllPages] = useState<Page[]>(initialData?.allPages || []);
    const [loading, setLoading] = useState(!initialData);
    const [widgetData, setWidgetData] = useState<Map<string, { rows: any[]; components: any[] }>>(() => {
